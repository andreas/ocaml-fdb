...
let run_network =
  foreign ~release_runtime_lock:true "fdb_run_network"
...
(* Future *)
let callback_t = fdb_future_t @-> ptr void @-> returning void
...
let error_predicate =
  foreign "fdb_error_predicate" (int @-> returning fdb_error_t)
